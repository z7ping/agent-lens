...
### 4.3 备份清单必须区分“逻辑资产”和“物理文件”

资产备份界面不能把扫描到的文件数量直接包装成用户理解中的逻辑资产数量。

例如：扫描到 4,000 个 MCP 相关文件时，只能显示为“4,000 个 MCP 相关文件”；只有 Source 或其他可靠事实能够证明具体 MCP 服务数量时，才允许显示“12 个 MCP 服务”。会话同理：如果当前只能可靠知道 123,053 个会话相关文件，就不得反推出 123,053 个会话。

因此备份概览允许同时提供：

- 逻辑资产数：仅在有可靠来源时展示；
- 物理文件数：始终来自备份索引；
- 数据大小：从现有备份索引中的文件大小汇总；
- 配置 / 数据根目录：从 Source 检测结果写入索引的路径语义推导；
- 最早 / 最近修改时间；
- 最近 30 天、31–90 天、91–180 天、180 天以前的物理文件分布；
- 各资产类别的文件数和大小。

这些解释性统计优先直接消费已有 `inventory-v1.json`，不得为了打开页面再次遍历第三方 Agent 的大目录。

### 4.4 备份决策以“数据价值”而不是“逐文件选择”为主

创建快照的默认交互按恢复价值分层：

- 建议备份：关键配置、Skill、MCP 配置/相关资产、Plugin、Extension、Hook、Rule 等；
- 按需备份：Session / History、Memory 等可能体积较大的历史数据；
- 默认排除：未分类或恢复价值不明确的数据；
- 敏感信息保护始终强制开启，不属于用户可关闭的策略。

界面需要在创建前给出影响预览，包括可可靠计算的文件引用数和大小。由于同一文件可能属于多个资产类别，分类统计允许存在重叠；真正创建快照时仍按物理路径去重。

AgentLens 1.0 的资产备份只负责“解释数据、帮助选择、创建/校验/导入快照、恢复预演”，不提供删除、移动、清理第三方 Agent 原始文件的能力。
...
- 逻辑资产数与物理文件数明确分离，避免把大量内部文件误导成大量 MCP / Session；
- 数据目录、占用空间和时间分布可以从既有索引解释，不需要重复扫描；
- 用户可以按恢复价值而不是逐文件做备份取舍；
- 导入包不能通过绝对路径任意写文件；
- 新旧快照格式保持兼容，不要求破坏性迁移；
...
- 对于 Source 尚未提供可靠逻辑资产数的类别，界面会显示“—”并只展示物理文件数；
