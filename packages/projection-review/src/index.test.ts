...
test('ReviewProjection localizes real lifecycle actions instead of collapsing them to one title', () => {
  ...
  assert.equal(label({ event: 'assistant.truncated', stopReason: 'length' }), '模型输出被截断')
  assert.equal(label({ event: 'assistant.error', stopReason: 'error' }), '模型响应错误')
  assert.equal(label({ event: 'assistant.cancelled', stopReason: 'aborted' }), '模型响应已取消')
  assert.equal(label({ event: 'assistant.stop', stopReason: 'length' }), '模型输出被截断')
