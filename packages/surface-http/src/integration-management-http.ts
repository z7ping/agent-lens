...
function preferencePayload(value: unknown): IntegrationPreferenceUpdateRequestDto {
  ...
  if (input.onboardingCompleted !== undefined) {
    if (input.onboardingCompleted !== true) {
      throw badRequest('onboardingCompleted may only transition to true')
    }
...
export async function handleIntegrationManagementRequest(
  ...
  if (!current.editable) {
    throw httpError(409, 'Integration Enabled state is managed by read-only runtime configuration')
  }
