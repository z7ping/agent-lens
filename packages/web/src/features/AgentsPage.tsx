...
function positiveStates(asset: AgentAssetInventoryDto): string[] {
  const states = new Set<string>()
  for (const binding of asset.bindings) {
    for (const state of binding.states) if (state.value === true) states.add(state.state)
  }
  return [...states]
}
...
function AssetCard({ agent, asset, priority = false }: { agent: AgentOverviewDto; asset: AgentAssetInventoryDto; priority?: boolean }) {
  ...
  const states = positiveStates(asset)
  return <div className={`asset-card ${priority ? 'asset-card-priority' : ''}`}>
    ...
    <div className="mt-2 flex min-h-5 flex-wrap gap-1.5">
      {states.length ? states.slice(0, 3).map(state => <span key={state} className="asset-state-dot">{stateLabel[state] ?? state}</span>) : <span className="text-[11px] text-muted">已发现</span>}
    </div>
...
function AgentCard({ agent }: { agent: AgentOverviewDto }) {
  ...
  return <article className="agent-overview-card" data-source={agent.sourceId}>
    ...
    <div className="agent-asset-summary">
      {grouped.length
        ? grouped.map(([type, items]) => <span key={type}>{assetTypeLabel[type] ?? type} <b>{items.length}</b></span>)
        : <span>暂无可识别资产</span>}
      {agent.usedAssets.length > 0 && <span className="agent-used-summary">已使用 <b>{agent.usedAssets.reduce((sum, item) => sum + item.callCount, 0)}</b></span>}
    </div>

    <section className="agent-section">
      <div className="agent-section-title">高频资产</div>
      {priorityAssets.length
        ...
        : <div className="overview-empty-line">暂无能够可靠归因的高频资产</div>}
    </section>

    <section className="agent-section space-y-2">
      {grouped.map(([type, assets]) => <details key={type} className="agent-group">
        <summary><span>{assetTypeLabel[type] ?? type}</span><span>{assets.length}</span></summary>
        <div className="asset-card-grid">{assets.slice(0, 24).map(asset => <AssetCard key={asset.id} agent={agent} asset={asset}/>)}</div>
      </details>)}
    ...
    <div className="agent-detail-grid">
      <details className="agent-group">
        ...
        <div className="assembly-list">
          ...
          {bindings.slice(0, 18).map(({ asset, binding }) => binding.path ? <div key={binding.id}><span>{assetTypeLabel[asset.type] ?? asset.type}</span><code>{binding.path}</code></div> : null)}
  ...
  return <main className="mx-auto max-w-[1800px]">
    ...
    <div className="p-6">
      <div className="mb-5">
        ...
        <p className="mt-1 text-sm text-muted">先看 Agent 当前状态，再看装了什么、怎么装配、哪些能力真正用过。</p>
      </div>
