...
const taskHeader = readFileSync(new URL('../../features/TaskHeader.tsx', import.meta.url), 'utf8')
...
test('Live Runtime diagnostics leave the ready Reader and reuse TaskHeader task-info Drawer', () => {
  assert.match(taskHeader, /infoContent\?: ReactNode/)
  assert.match(taskHeader, /\{infoContent && <div className="task-header-info-content">\{infoContent\}<\/div>\}/)
  assert.match(liveTaskPage, /infoContent=\{<LiveRuntimeTaskInfo/)
  assert.match(liveTaskPage, /state\?\.status === 'failed' && <LiveRuntimeFailureNotice/)
  assert.doesNotMatch(liveTaskPage, /<LiveRuntimeDisclosures/)
  assert.match(liveRuntimeDisclosures, /import \{ Button, Disclosure \} from '\.\/ui'/)
  assert.doesNotMatch(liveRuntimeDisclosures, /\bDialog\b|live-runtime-detail-overlay/)
  assert.match(liveRuntimeDisclosures, /className="live-runtime-info-section"/)
  assert.match(liveRuntimeDisclosures, /className="live-runtime-info-values"/)
  assert.match(liveRuntimeDisclosures, /Advanced diagnostics|高级诊断/)
  assert.match(piLiveCss, /\.live-runtime-task-info\s*\{/)
  assert.match(piLiveCss, /\.live-runtime-info-values\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/)
  assert.doesNotMatch(piLiveCss, /live-runtime-detail-overlay|live-runtime-detail-overview|live-runtime-disclosure-compact/)
