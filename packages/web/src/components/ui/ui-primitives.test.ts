...
test('Live Runtime keeps lifecycle in Reader and presents polished copyable diagnostics', () => {
  ...
  assert.match(liveRuntimeDisclosures, /window\.setInterval\(\(\) => setClock\(Date\.now\(\)\), 250\)/)
  assert.match(liveRuntimeDisclosures, /timingBaseline\.current\.activeDurationMs \+ liveDeltaMs/)
  assert.match(liveRuntimeDisclosures, /duration\(elapsedMs\)/)
  assert.match(piStartupCss, /@keyframes pi-startup-pulse/)
  assert.match(piStartupCss, /@keyframes pi-startup-active-ring/)
  assert.match(piStartupCss, /@media \(prefers-reduced-motion: reduce\)/)
