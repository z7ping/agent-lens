...
test('Workspace Shell owns three primary workspaces and one dynamic context host', () => {
  ...
  assert.match(workspacePrimaryNavigation, /id: 'review',[\s\S]*?to: '\/review',[\s\S]*?label: '任务'/)
  assert.match(workspacePrimaryNavigation, /id: 'insights',[\s\S]*?to: '\/insights',[\s\S]*?label: '洞察'/)
  assert.match(workspacePrimaryNavigation, /id: 'agents',[\s\S]*?to: '\/agents',[\s\S]*?label: '智能体'/)
  ...
  assert.match(workspaceSidebar, /aria-label="收起侧栏"/)
  assert.match(app, /aria-label="展开侧栏"/)
...
test('Task Center context owns search, controlled filters and task list', () => {
  ...
  assert.match(taskCenterPage, /className="workspace-insight-filter-fields" aria-label="筛选历史任务"/)
...
test('Review, Backup and Agent Overview retain their valid local primitives', () => {
  ...
  assert.match(agentsPage, /import \{ UiIcon \} from '\.\.\/components\/ui'/)
