...
test('Paginated UserMessage is a genuine user turn with item identity', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'UserMessage', id: 'user-item-1', client_id: 'client-1',
    content: [
  ...
  assert.equal(payload.text, '继续修复 Codex')
  assert.equal(provenance.actualAuthor, 'human-user')
  assert.equal(provenance.contentRole, 'user-request')
  assert.equal(provenance.sourceSignal, 'event_msg.item_completed.UserMessage')
  assert.equal(payload.turnId, 'turn-1')
  assert.equal(Array.isArray(payload.attachments) ? payload.attachments.length : 0, 1)
})

test('Paginated AgentMessage is normal assistant output, not background unknown', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'AgentMessage', id: 'agent-item-1', content: [{ type: 'Text', text: '正常回复内容' }], phase: 'final_answer',
  }), ctx)
  assert.ok(output)
  const fact = output.observations[0]!
  const payload = asRecord(fact.payload)
  ...
  assert.equal(payload.text, '正常回复内容')
  assert.equal(provenance.actualAuthor, 'assistant')
  assert.equal(provenance.activityType, 'conversation')
  assert.equal(provenance.sourceSignal, 'event_msg.item_completed.AgentMessage')
  assert.equal(payload.turnId, 'turn-1')
})

test('Paginated Plan is assistant commentary rather than background unknown', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({ type: 'Plan', id: 'plan-item-1', text: '先检查协议，再修改映射。' }), ctx)
  assert.ok(output)
  const fact = output.observations[0]!
  const payload = asRecord(fact.payload)
  const provenance = asRecord(payload.provenance)
  ...
  assert.equal(payload.text, '先检查协议，再修改映射。')
  assert.equal(payload.plan, true)
  assert.equal(provenance.actualAuthor, 'assistant')
  assert.equal(provenance.sourceSignal, 'event_msg.item_completed.Plan')
})

test('Paginated HookPrompt stays application context and never becomes a user turn', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'HookPrompt', id: 'hook-item-1', fragments: [{ text: 'Injected by hook', hookRunId: 'hook-run-1' }],
  }), ctx)
  assert.ok(output)
  const fact = output.observations[0]!
  const provenance = asRecord(asRecord(fact.payload).provenance)
  assert.equal(fact.kind, 'context.injected')
  assert.equal(fact.nativeEventId, 'hook-item-1')
  assert.equal(provenance.actualAuthor, 'application')
  assert.equal(provenance.contentRole, 'application-context')
  assert.equal(provenance.sourceSignal, 'event_msg.item_completed.HookPrompt')
...
test('Paginated Reasoning becomes visible Thinking content', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'Reasoning', id: 'reasoning-item-1', summary_text: ['先检查协议。', '再修投影。'], raw_content: ['private raw detail'],
  }), ctx)
  assert.ok(output)
  const fact = output.observations[0]!
  const payload = asRecord(fact.payload)
  ...
  assert.equal(payload.text, '先检查协议。\n\n再修投影。')
...
test('Paginated CommandExecution becomes one tool call/result pair', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    ...
    parsed_cmd: [], source: 'agent', status: 'completed', stdout: 'all tests passed', stderr: null, exit_code: 0,
  }), ctx)
  assert.ok(output)
  ...
  assert.equal(callPayload.nativeToolName, 'command_execution')
  assert.equal(callPayload.callId, undefined)
  assert.equal(call?.nativeCallId, undefined)
  assert.equal(result?.nativeCallId, undefined)
  assert.equal(call?.dedupHints?.sharedEventKey, 'codex-call:r-1')
  assert.equal(result?.dedupHints?.sharedEventKey, 'codex-call:r-1')
  assert.equal(resultPayload.success, true)
  assert.equal(resultPayload.output, 'all tests passed')
  assert.equal(resultPayload.turnId, 'turn-1')
})

test('Paginated FunctionCallOutput is a tool result instead of raw background activity', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'FunctionCallOutput', id: 'function-output-1', name: 'read_file', namespace: 'workspace', output: { text: 'file content' },
  }), ctx)
  assert.ok(output)
  const fact = output.observations[0]!
  const payload = asRecord(fact.payload)
...
test('Paginated CollabAgentToolCall remains a visible tool operation', async () => {
  const output = await normalizePaginatedCodexRecord(itemCompleted({
    ...
    sender_thread_id: 'thread-root', receiver_thread_ids: ['thread-child'], receiver_agents: [],
    prompt: '检查测试', agents_states: { 'thread-child': 'completed' },
  }), ctx)
  assert.ok(output)
  assert.deepEqual(output.observations.map(item => item.kind), ['tool.call', 'tool.result'])
  assert.equal(asRecord(output.observations[0]?.payload).nativeToolName, 'collab_agent.spawn_agent')
  assert.equal(asRecord(output.observations[1]?.payload).success, true)
})
...
test('Paginated SubAgentActivity maps native lifecycle without becoming unknown', async () => {
  const started = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'SubAgentActivity', id: 'subagent-start-1', kind: 'started', agent_thread_id: 'thread-child', agent_path: ['worker'],
  }), ctx)
  const interacted = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'SubAgentActivity', id: 'subagent-interact-1', kind: 'interacted', agent_thread_id: 'thread-child', agent_path: ['worker'],
  }, 2), ctx)
  const completed = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'SubAgentActivity', id: 'subagent-end-1', kind: 'completed', agent_thread_id: 'thread-child', agent_path: ['worker'],
  }, 3), ctx)
  ...
  assert.equal(asRecord(interacted.observations[0]?.payload).event, 'subagent.interacted')
...
test('Paginated ReviewMode is explicit lifecycle and does not reclassify the main session', async () => {
  const entered = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'EnteredReviewMode', id: 'review-entered-1', target: { type: 'uncommittedChanges' }, user_facing_hint: '正在审查',
  }), ctx)
  ...
  const enteredPayload = asRecord(entered.observations[0]?.payload)
  const exitedPayload = asRecord(exited.observations[0]?.payload)
  assert.equal(entered.observations[0]?.kind, 'session.lifecycle')
  assert.equal(enteredPayload.event, 'review.entered')
  assert.equal(enteredPayload.sessionActivity, undefined)
  assert.equal(exitedPayload.event, 'review.exited')
})
...
test('Paginated FileChange and image items are artifact actions', async () => {
  const file = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'FileChange', id: 'file-change-1', changes: { 'src/a.ts': { type: 'update', unified_diff: '@@' } }, status: 'completed', auto_approved: true,
  }), ctx)
  const view = await normalizePaginatedCodexRecord(itemCompleted({ type: 'ImageView', id: 'image-view-1', path: 'file:///safe/image.png' }, 2), ctx)
  const generated = await normalizePaginatedCodexRecord(itemCompleted({
    type: 'ImageGeneration', id: 'image-generation-1', status: 'completed', revised_prompt: 'a diagram', result: 'image-data', saved_path: '/safe/generated.png',
  }, 3), ctx)
  assert.ok(file && view && generated)
  assert.equal(file.observations[0]?.kind, 'artifact.action')
  assert.equal(asRecord(file.observations[0]?.payload).action, 'file.change')
  assert.equal(view.observations[0]?.kind, 'artifact.action')
  assert.equal(asRecord(view.observations[0]?.payload).action, 'image.view')
  assert.equal(generated.observations[0]?.kind, 'artifact.action')
  assert.equal(asRecord(generated.observations[0]?.payload).action, 'image.generate')
})
...
test('Paginated item identity uses TurnItem.id instead of shared turn_id', () => {
  const first = {
    type: 'event_msg',
    payload: { type: 'item_completed', turn_id: 'same-turn', item: { type: 'AgentMessage', id: 'agent-a', content: [] } },
  }
  const second = {
    type: 'event_msg',
    payload: { type: 'item_completed', turn_id: 'same-turn', item: { type: 'AgentMessage', id: 'agent-b', content: [] } },
  }
