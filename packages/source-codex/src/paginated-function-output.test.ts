...
test('Paginated FunctionCallOutput becomes canonical tool result', async () => {
  ...
  assert.equal(fact.nativeCallId, undefined)
  assert.equal(fact.dedupHints?.sharedEventKey, 'codex-call:function-output-record')
  assert.equal(payload.callId, undefined)
