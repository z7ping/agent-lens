...
test('runtime profiles resolve stably and attach to canonical sessions', async () => {
  ...
  try {
    ...
    const first = await storage.runtimeProfiles.resolve({
      installationId: 'install-dsh',
      nativeProfileId: 'default',
      name: 'default',
      dataRoot: '/home/user/.deepseek/dsh/profiles/default',
    })
    const second = await storage.runtimeProfiles.resolve({
      installationId: 'install-dsh',
      nativeProfileId: 'default',
      name: 'default',
      dataRoot: '/home/user/.deepseek/dsh/profiles/default',
    ...
    const sourceRow = storage.db.prepare(
      'SELECT runtime_profile_id FROM source_sessions WHERE id = ?',
    ).get('source-1') as { runtime_profile_id: string | null }
    const logicalRow = storage.db.prepare(
      'SELECT runtime_profile_id FROM logical_sessions WHERE id = ?',
    ).get('logical-1') as { runtime_profile_id: string | null }
...
test('source runtime status preserves last error and accumulates failures', async () => {
  ...
  try {
    ...
    await storage.sourceRuntimeStatus.put({
      ...base,
      state: 'failed',
      lastErrorAt: '2026-08-25T10:01:00.000Z',
      lastErrorSummary: 'first failure',
    })
    await storage.sourceRuntimeStatus.put({
      ...base,
      state: 'healthy',
      lastSuccessAt: '2026-08-25T10:02:00.000Z',
    })
    await storage.sourceRuntimeStatus.put({
      ...base,
      state: 'failed',
      lastErrorAt: '2026-08-25T10:03:00.000Z',
      lastErrorSummary: 'second failure',
    })
