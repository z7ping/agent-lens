  ...
  return `
    WITH session_aggregates AS (
      SELECT
        logical_session_id,
        COALESCE(
          MIN(CASE WHEN occurred_at IS NOT NULL OR kind <> 'session.lifecycle' THEN COALESCE(occurred_at, captured_at) END),
          MIN(captured_at)
        ) AS started_at,
        COALESCE(
          MAX(CASE WHEN occurred_at IS NOT NULL OR kind <> 'session.lifecycle' THEN COALESCE(occurred_at, captured_at) END),
          MAX(captured_at)
        ) AS ended_at,
...
function summaryQueryWhere(input: SessionSummaryQuery): { sql: string; params: unknown[] } {
  ...
  if (input.sourceIds !== undefined) {
    ...
    conditions.push(`EXISTS (
      ...
      WHERE source.value IN (${input.sourceIds.map(() => '?').join(', ')})
    )`)
