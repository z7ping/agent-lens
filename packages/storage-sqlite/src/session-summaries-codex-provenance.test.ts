...
function attachNativeEvidence(storage: SqliteStorageService, observationId: string, installationId: string, nativeType: string, suffix: string): void {
  ...
  storage.db.prepare(`
    ...
    ) VALUES (?, 'codex', ?, 'legacy-codex-session', ?, ?, '{}', '{}', 'legacy')
  `).run(`source-${suffix}`, installationId, nativeType, capturedAt)
  storage.db.prepare(`
