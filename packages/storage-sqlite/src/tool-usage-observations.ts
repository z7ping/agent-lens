...
export class SqliteToolUsageObservationReader implements ToolUsageObservationReader {
  ...
  aggregate(input: ToolUsageAggregateQuery): Promise<ToolUsageAggregateResult> {
    return this.executor.run(() => {
      ...
      // SQLite 的 CTE 作用域仅限一条语句。将所有聚合分支放进同一条复合查询，
      // 让 events/calls/tool_events 的物化结果真正共享，避免每个下钻维度都重新扫描
      // observations 并重复解析 payload JSON。
      ...
      for (const row of rows) {
        ...
        if (row.row_kind === 'tool') {
          ...
          tools.set(`${sourceId}\u0000${toolName}`, {
            ...
            sourceIds: [sourceId],
            productIds: Array.isArray(payload.productIds) ? payload.productIds.filter((item): item is string => typeof item === 'string').sort() : [],
          ...
          assets.set(`${assetType}\u0000${canonicalName}`, {
            ...
            canonicalName,
            sourceIds: Array.isArray(payload.sourceIds) ? payload.sourceIds.filter((item): item is string => typeof item === 'string').sort() : [],
            callCount: Number(payload.callCount ?? 0),
            firstUsedAt: String(payload.firstUsedAt),
            lastUsedAt: String(payload.lastUsedAt),
            observationIds: [],
