...
import { SqliteHubReplicaStore, SqliteStorageService } from '@agent-lens/storage-sqlite'
import { activateReplicaGeneration, importReplicationBatch } from './importer'
...
test('H7 rolls back entities, shared identity, committed batch and ACK when one entity fails Hub recomputation', async () => {
  ...
  try {
    ...
    await assert.rejects(
      importReplicationBatch({ store, batch: second }),
      /shared key mismatch/,
    )
