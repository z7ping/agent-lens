...
test('discovery service exposes scanning state and coalesces concurrent rescans', async () => {
  ...
  assert.equal(completed.items.length, 6)
...
test('discovery service can rescan one integration without replacing other discovery results', async () => {
  ...
  assert.equal(scoped.items.length, 6)
...
test('DSH is reported as data-only from a profiles root without requiring an executable', async () => {
