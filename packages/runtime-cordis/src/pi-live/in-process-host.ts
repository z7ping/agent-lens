...
class InProcessHandle implements PiRuntimeHandle {
  ...
  private modelSnapshot(provider?: string): readonly PiSdkModel[] { const snapshot = this.session.modelRuntime.getAvailableSnapshot(); return provider ? snapshot.filter(model => model.provider === provider) : snapshot }
