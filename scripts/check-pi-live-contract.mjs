...
requireText(taskCenter, /nativeDirectoryPicker && <div className="task-center-new-directory-actions">[\s\S]{0,700}selectDirectoryAndStart/, '桌面主流程必须优先使用原生目录选择器')
requireText(taskCenter, /const manualDirectoryVisible = !nativeDirectoryPicker \|\| manualDirectoryOpen/, '手输路径只能作为无原生选择器或用户主动展开的备用入口')
...
requireText(page, /<TaskHeader[\s\S]{0,5200}actions=\{<>[\s\S]{0,900}\{optimisticStreaming && <Button[\s\S]{0,500}t\('header\.interrupt'\)/, 'Pi Live 标题和执行态中断控制必须由 TaskHeader 渲染')
...
requireText(page, /import \{ PiLiveCurrentTaskRound, PiLiveHistoryTaskRound \} from '\.\/PiLiveTaskRound'/, 'Pi Live 必须通过统一 Current / History Adapter 进入共享 Round')
...
requireText(page, /const PiLiveHistoryVirtualRound = memo\(function PiLiveHistoryVirtualRound/, '稳定历史轮次必须通过 memo 隔离 Composer 父级更新')
requireText(page, /visibleHistoryRounds\.map[\s\S]{0,900}<PiLiveHistoryVirtualRound/, '普通历史轮次必须通过 memoized VirtualRoundMount 包装')
requireText(page, /PiLiveHistoryVirtualRound[\s\S]{0,900}<PiLiveHistoryTaskRound/, 'memoized 历史轮次必须继续复用 PiLiveHistoryTaskRound')
requireText(page, /<PiLiveCurrentTaskRound[\s\S]{0,520}items=\{currentItems\}/, '当前轮次必须通过共享有序内容块 Adapter')
...
requireText(piTaskRound, /export function PiLiveCurrentTaskRound[\s\S]{0,1800}<TaskRound/, 'Pi Current 必须复用 TaskRound')
requireText(piTaskRound, /streaming=\{entry\.role === 'assistant' && entry\.state === 'running'\}/, '实时与完成态助手正文必须复用 TaskMessage')
if (/pi-live-stream-response|PiLiveRunningTaskRound/.test(`${page}\n${piTaskRound}\n${css}`)) failures.push('不得恢复平行 Streaming Tail 与旧 Running Adapter')
...
requireText(page, /PiLivePresentationScheduler/, '缺少流式展示批处理器')
requireText(page, /presentation\.push\(items => appendPiLiveDelta/, 'Text/Thinking delta 必须进入展示批处理')
requireText(taskMessage, /streaming && !user && view === 'rendered'/, '流式 Assistant 不得重复执行折叠测量')
...
requireText(runtime, /if \(input\.sessionPath && input\.historyAction !== 'fork'\)[\s\S]{0,700}if \(duplicate\) return this\.runtimeState\(duplicate\)/, 'Runtime 必须幂等复用已经打开的同一原生 Pi 会话，禁止重复 Runtime')
...
if (!taskCenterCss.includes('767.98px')) failures.push('任务中心页面壳层 CSS 缺少移动端响应式基线 767.98px')
...
console.log('Pi Live / 任务中心契约检查通过：唯一 TaskSurface 会话宿主、三大工作区导航、原生目录选择主路径、幂等会话恢复、独立 SDK Worker、异步状态机、有界 IPC、队列/Abort、历史事实、IME、滚动跟随与响应式布局已锁定。')
