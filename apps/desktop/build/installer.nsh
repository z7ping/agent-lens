...
; 4. Desktop 安装后提供 agent-lens.cmd，并把当前安装目录登记到当前用户 PATH；卸载时只移除该目录，不改 npm 全局安装。
;
...
!macro customInstall
  ...
  nsExec::ExecToLog '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\agent-lens-cli-path.ps1" -Action install -InstallDir "$INSTDIR"'
  Pop $0
  ${If} $0 != 0
...
!macro customUnInstall
  ; 升级时同样先清掉旧安装目录 PATH，随后新安装阶段会幂等加入当前目录；
  ; 这样允许未来改变安装目录而不留下陈旧 PATH。
  File /oname=$PLUGINSDIR\agent-lens-cli-path.ps1 "${BUILD_RESOURCES_DIR}\agent-lens-cli-path.ps1"
