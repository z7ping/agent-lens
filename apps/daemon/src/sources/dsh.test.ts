...
test('DSH 冷导入按最近活动时间优先并稳定回填旧文件', async () => {
  const root = await mkdtemp(join(tmpdir(), 'agent-lens-dsh-order-'))
  const sessions = join(root, 'sessions')
  ...
  try {
    assert.deepEqual(await dshSourceInternals.sessionFiles(root), [
      recentBPath,
      recentAPath,
      oldPath,
    ])
