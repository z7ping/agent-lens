...
test('容量受限时停止历史扩张，但始终保留严格有界的最新会话发现', () => {
  ...
  assert.deepEqual(stagesAllowedByCapacity(stages, 'unknown').map(stage => stage.id), ['latest'])
  assert.deepEqual(stagesAllowedByCapacity(stages, 'exceeded')[0]?.window, { sessionLimit: 1 })
  assert.deepEqual(stagesAllowedByCapacity(stages, 'unknown')[0]?.window, { sessionLimit: 1 })
