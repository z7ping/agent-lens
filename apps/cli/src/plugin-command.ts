...
function listLine(item: IntegrationManagementItemDto): string {
  return [
...
function printItem(item: IntegrationManagementItemDto, print: (line: string) => void): void {
  print(`${item.displayName} (${item.integrationId})`)
  print(`  本机工具：${toolLabel(item)}`)
...
export async function runPluginCommand(
  ...
  if (action === 'list') {
    ...
    const result = await management(options)
    if (json) {
      print(JSON.stringify({ items: result.items, meta: result.meta }, null, 2))
      return 0
    }
    ...
    result.items.forEach(item => print(listLine(item)))
    return 0
  }
  ...
  if (action === 'status') {
    ...
    const result = await management(options)
    const item = result.items.find(candidate => candidate.integrationId === integrationId)
    if (!item) throw new Error(`未找到官方 Integration：${integrationId}`)
    if (json) print(JSON.stringify({ item, meta: result.meta }, null, 2))
    else printItem(item, print)
    return 0
  }
