...
涉及外部工具、协议、Runtime、SDK、格式或第三方能力接入时，**写实现之前必须先完成现成方案调研**，至少按以下顺序检查：

1. 官方文档、官方 SDK / API、官方发行包；
2. 上游官方 GitHub 仓库及其示例、测试和参考实现；
3. npm 等正式包管理渠道中成熟、持续维护的现成实现；
4. 只有前述方案不能满足需求时，才进入自研协议、Runtime 或兼容层评估。

调研后必须明确选择并说明理由：**直接复用 / 薄适配封装 / 参考实现 / 自研**。默认优先级为“官方受支持实现 > 成熟维护实现 > 参考实现 > 自研”。如果选择自研，必须说明官方/现成方案在功能、稳定性、安全与隐私、性能、平台兼容、许可证或维护成本上的具体缺口。

不得在没有完成上述调研和取舍记录的情况下，重复实现已有官方 SDK/API 所覆盖的协议或 Runtime。薄适配层应尽量保持 AgentLens 自身 Contract 稳定，避免把第三方实现细节扩散到 Web、Storage 或其他 Source。
