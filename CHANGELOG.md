...
- 新增全局后台活动状态，在主界面可直接看到 Pi 历史同步、数据索引等后台任务及最近活动，不再只能通过日志判断系统是否仍在工作。
- Pi Live 新建任务支持项目目录选择；Desktop 使用系统原生目录选择器，浏览器 / CLI Web 保留 Daemon 目录选择回退。
- Pi Live 增加任务摘要，并为持久化状态设置载荷上限，降低超长任务恢复数据持续膨胀的风险。
- 任务与洞察筛选支持多来源组合选择，智能体筛选交互同步收敛。

### 界面与交互

- 历史复盘与 Pi Live 统一到同一套 Session View（会话视图），共享任务头、正文阅读轴、轮次导航、滚动区域与输入区几何，减少实时与历史两套界面继续漂移。
- 统一 Review 与 Pi Live 的轮次导轨语义和边界导航，并恢复成熟的悬停、选中、运行中和异常状态表现。
- 资产备份页面恢复独立纵向滚动，长内容不再被页面布局截住。
- 收敛 Pi Live 任务头和阅读区布局，修复输入框与上下边界导航在窗口模式下重叠等界面问题。

### 稳定性与修复

- Pi Live 支持 Daemon 重启后恢复仍在运行的任务，不会重新提交原 Prompt；Runtime 显式终止后会清理恢复状态，避免已结束任务被错误恢复。
- 修复 Pi 历史会话继续 / 分叉相关的幂等与 Session 身份边界，重复继续不会重复创建或错误占用会话。
- 修复 Ubuntu / Volta 等环境下 Pi 可执行文件与 Official Pi SDK 解析不一致导致初始化卡在“加载 Pi SDK”的问题，并加强项目工作目录校验。
- 修复 Review、使用洞察和工具详情的筛选比较逻辑，项目、时间范围、状态、搜索条件变化不再因来源选择引用相同而被跳过。
- Windows 后台任务改为隐藏 PowerShell 启动；npm 更新时会迁移已注册的旧任务并保留自启 / 运行语义，后台日志改为 2 MiB 有界轮转。
- 修复 Session Summary Projection 注销时未释放 registration 的生命周期问题，允许同一 Projection ID 在 dispose 后安全重新注册。
- 修复 Pi 会话展示、智能体概览加载、DeepSeek 来源标识以及若干严格类型检查暴露的界面回归。

### 架构与发行

- 统一 Codex / Claude / Pi 的 Source Home、Hook 配置和 CLI `setup` / `doctor` 路径解析；Pi Source 同时复用统一可执行文件发现，减少不同入口对本机环境理解不一致。
- Desktop 原生发行包统一为 **x64-only**：Windows、macOS、Linux 不再发布 ARM64 原生包；npm / CLI 分发不受此限制。Apple Silicon 仍可通过 Rosetta 运行 x64 macOS 包。
- PR 校验工作流不再因 PR 内每次普通提交自动重复触发，保留打开、重新打开、转为待评审和手动触发，减少无意义 CI 消耗。

## 1.0.0-alpha.4（2026-09-08）

### 修复

- 修复 Session Summary 在事务提交后触发异步刷新时，偶发使用已失效 Data Runtime 事务标识并持续报错的问题。
- 修复 Pi 历史会话分叉后仍占用原会话路径的问题；分叉 Runtime 现在会锁定新生成的会话文件，原会话可继续打开，同时避免新分支被重复占用。
- 修复 Pi Runtime 在初始化极快时直接显示为“就绪”的竞态；现在创建任务后会先明确显示“初始化中”，再切换到就绪或失败状态。
- 修复混合历史导入中，只有采集时间的后续元数据可能把旧会话错误排到导入当天的问题；已有真实事件时间时，任务时间继续以真实事件时间为准。

### 稳定性

- 生产环境下重复的 Data Runtime 与 Session Summary 诊断错误改为限频记录，保留首条和周期性摘要，避免同一故障快速刷满日志。
- 桌面端 Daemon 与启动日志增加大小轮转和有限历史保留，降低长时间运行后的磁盘占用风险。
- Web 统一组件样式改由正式入口集中加载，保持页面样式一致，并使组件级渲染测试不再依赖浏览器 CSS 加载能力。
...
- Pi 历史会话新增 **继续会话** 和 **分叉继续**：可以选择在原会话中继续，也可以保留原历史并从当前节点创建新的分支会话。
- 任务详情新增 **核心事件 / 全部事件** 查看方式。日常使用默认只展示重要信息，需要排查问题时仍可查看完整事件和原始证据。
- 桌面端、npm / CLI Web 增加新版本提示，不再需要用户频繁手工检查是否有新版本。
...
- 优化 Pi 实时任务输入体验：中文输入法组合输入时不会误发送；输出过程中仍可继续输入并排队；中断后未执行的输入可以恢复编辑；阅读历史时，新输出不会强制把页面拉回底部。
- 长会话滚动和历史阅读体验进一步优化，切换任务或返回历史位置时减少跳动和定位丢失。

### 任务复盘与数据展示

- 修正历史会话时间语义，任务排序优先使用智能体真实产生事件的时间，不再因为重新采集或导入而把旧任务错误排到今天。
- 会话标题规则统一：优先使用智能体原生标题或摘要，其次使用首条有效用户输入，列表和详情不再出现标题不一致。
- Pi 恢复原生事件父子关系与分支结构，任务复盘能够更准确地还原真实会话过程。
- Pi 实时任务和历史复盘对同一会话的展示进一步统一，减少“实时看到一种结果，回到历史又变成另一种结果”的情况。
- Codex 的用户输入、系统注入、开发者上下文和客户端机器信息重新区分，机器生成的上下文不再错误显示成真人用户消息。
- Codex 最终回复中的内部机器标记、客户端指令等不再混入正常回复正文、标题和预览。
- 用量、模型变化、推理配置、上下文压缩、子智能体通信等更多智能体原生信息现在可以进入统一任务视图。
- 对暂时还没有专门界面的原生事件，不再静默丢失；可以通过“全部事件”和原始事件详情查看。
...
- 修复部分任务复盘辅助数据请求失败时，整个任务正文也无法显示的问题；现在辅助信息失败不会影响主要内容阅读。
- 修复项目、时间范围、使用洞察在历史数据较多时只统计部分记录的问题。
- 优化工具分析和智能体概览的大数据查询，避免随着历史数据增长越来越慢。
- 数据库重任务与前台交互进一步隔离，历史解析回放、统计回填和后台维护不会再轻易把任务中心和 Pi 实时交互一起拖慢。
- 增加前台查询排队和重复请求合并，高并发或多个页面同时刷新时更不容易出现请求失败。
- 工具统计增加完整性状态，后台数据尚未回填完成时会明确标记为部分结果，而不是展示一个看似完整但实际上缺失的数据集。
- 在约 1.59 GB 实际数据库上完成大库收口：工具事实完整回填达到 50,723 / 50,723；工具分析 64 个并发请求全部成功；包含多类页面查询的 128 个混合并发请求全部成功，并修复此前高并发下可能出现的大面积请求失败问题。

### 修复

- 修复 Pi 实时任务从发送消息到 AI 开始输出期间界面明显抖动的问题，并进一步平滑状态切换。
- 修复 Pi 实时任务队列、状态提示、模型和推理等级等信息频繁增减高度造成的布局跳动。
- 修复 Pi 实时任务切换运行时、刷新、重连过程中，旧任务数据偶尔覆盖当前任务的问题。
- 修复用户正在阅读历史时，新消息强制抢滚动的问题。
- 修复超长任务、超长单轮工具调用导致单次响应无限增长的问题。
- 修复工具统计反复扫描大量历史数据，导致页面加载和运行时响应变慢的问题。
- 修复大数据库下历史解析回放、健康检查、后台回填相互争用造成的周期性卡顿。
- 修复部分数据来源未识别的新字段或新事件在进入 AgentLens 后直接丢失的问题；现在会在安全处理后继续保存原始证据。
- 修复 Codex 部分原生持久化事件被错误归类为未知事件，或可见正文与机器元数据混杂的问题。
- 修复部分页面重复导航、重复标题、重复工具栏和低频信息长期占据主要空间的问题。

### 已知限制

- `1.0.0-alpha.3` 仍属于预发布版本，后续仍会继续观察更长时间运行和更大历史数据量下的资源变化。
- 当前 Pi 实时交互已经比较完整，但 AgentLens 暂不负责自动升级用户机器上的 Pi 运行时。
